...
void initKingMovementTable(void) {
    for (int sq = 0; sq < 64; sq++) {
        ...
        if (sq <= A7) moves |= 1LL << (sq+8); // Arriba
        if (sq >= H2) moves |= 1LL << (sq-8); // Abajo
        if (file < A) moves |= 1LL << (sq+1); // Izquierda
        if (file > H) moves |= 1LL << (sq-1); // Derecha
        if (file < A && sq <= A7) moves |= 1LL << (sq+9); // Arriba izquierda
        if (file > H && sq <= A7) moves |= 1LL << (sq+7); // Arriba derecha
        if (file < A && sq >= H2) moves |= 1LL << (sq-7); // Abajo izquierda
        if (file > H && sq >= H2) moves |= 1LL << (sq-9); // Abajo derecha
...
void initKnightMovementTable(void) {
    for (int sq = 0; sq < 64; sq++) {
        ...
        if (file != A && sq <= A6) { bb |= 1LL << sq+17;} // Arriba izquierda
        if (file != H && sq <= A6) { bb |= 1LL << sq+15;} // Arriba derecha
        if (file != A && sq >= A2) { bb |= 1LL << sq-15;} // Abajo izquierda
        if (file != H && sq > H3) { bb |= 1LL << sq-17;} // Abajo derecha
        if (file < B && sq <= A7) { bb |= 1LL << sq+10;} // Izquierda arriba
        if (file < B && sq >= H2) { bb |= 1LL << sq-6;} // Izquierda abajo
        if (file > G && sq >= H2) { bb |= 1LL << sq-10;} // Derecha abajo
        if (file > G && sq <= A7) { bb |= 1LL << sq+6;} // Derecha arriba
...
void initPawnAttackTables(void) {
    for (int sq = 0; sq < 64;sq++) {
        ...
        Bitboard attack = 0;
        if (file != A && sq <= H8) attack |= 1LL << (sq+9); 
        PAWN_W_ATTACKS_WEST[sq] = attack;

        attack = 0;
        if (file != H && sq <= H8) attack |= 1LL << (sq+7); 
        PAWN_W_ATTACKS_EAST[sq] = attack;

        attack = 0;
        if (file != H && sq >= H2) attack |= 1LL << (sq-9); 
        PAWN_B_ATTACKS_WEST[sq] = attack;

        attack = 0;
        if (file != A && sq >= H2) attack |= 1LL << (sq-7); 
